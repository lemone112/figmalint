  ...
  if (!score && miniScore) {
    return (
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-border bg-bg-secondary">
        <span className={`w-2 h-2 rounded-full ${getSeverityDot(miniScore.topSeverity)}`} role="img" aria-label={`Severity: ${miniScore.topSeverity}`} />
        ...
        <div className="flex items-center gap-2">
          ...
          {trend !== null && trend !== 0 && (
            <span className={`text-11 font-medium ${trend > 0 ? 'text-fg-success' : 'text-fg-danger'}`} title={`${trend > 0 ? '+' : ''}${trend} vs baseline`}>
              {trend > 0 ? (
                <ChevronUp size={12} className="inline" aria-hidden="true" />
              ) : (
                <ChevronDown size={12} className="inline" aria-hidden="true" />
              )}
              {trend > 0 ? '+' : ''}{trend}
            </span>
          )}
